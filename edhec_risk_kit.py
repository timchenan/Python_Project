...
def get_ffme_returns():
    me_m = pd.read_csv(r"C:\Users\test\Desktop\Github\Intro to Portfolio Consturuction\data sources\data\Portfolios_Formed_on_ME_monthly_EW.csv", 
...
def get_hfi_returns():
    ...
    """
    hfi = pd.read_csv(r"C:\Users\test\Desktop\Github\Intro to Portfolio Consturuction\data sources\data\edhec-hedgefundindices.csv",
                      header=0, index_col=0, parse_dates=True, na_values=-99.999)
    hfi=hfi/100
...
def get_ind_size():
    """
    """
    ind = pd.read_csv(r'C:\Users\test\Desktop\Github\Intro to Portfolio Consturuction\data sources\data\ind30_m_size.csv',
...
def get_ind_nfirms():
    """
    """
    ind =pd.read_csv(r'C:\Users\test\Desktop\Github\Intro to Portfolio Consturuction\data sources\data\ind30_m_nfirms.csv',
                header=0, index_col=0, parse_dates=True)
    ind.index=pd.to_datetime(ind.index, format='%Y%m').to_period('M')
    ind.columns=ind.columns.str.strip()
    return ind
...
def get_ind_returns():
    ...
    """
    ind =pd.read_csv(r'C:\Users\test\Desktop\Github\Intro to Portfolio Consturuction\data sources\data\ind30_m_vw_rets.csv',
...
import scipy.stats
def is_nrormal(r, level=0.01):
    """
    Applies the Jarque-Bera test to determine if a series is normal or not
    Test is applied at the 1% level by default
    Returns True if the hypothesisof normality is accepted, False otherwise
    """
...
def cvar_historic(r, level=5):
    """
    Compute the Condition VaR of Series or DataFrame
    """
    if isinstance(r, pd.Series):
        ...
        raise TypeError("Expect r to be a Series or DataFrame")
...
def optimal_weights(n_points, er, cov):
    """
    -> list of weights to run optimizer on to minimize the vol
    """
...
def msr(riskfree_rate, er, cov):
    ...
    given the riskfree rate and expected returnsand a covariance matrix
    """
    n = er.shape[0]
    init_guess = np.repeat(1/n, n)
    bounds = ((0.0, 1),)*n
...
def run_cppi(risky_r, safe_r=None, m=3, start=1000, floor=0.8, riskfree_rate=0.03, drawdown=None):
    """Run a backtest of the CPPIstrategy, given a set of returns for the risky asset
